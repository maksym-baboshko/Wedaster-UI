...
import { Button, Card, Input } from "@wedaster/ui"
```

`@wedaster/ui/base.css` is optional. Use it only if you want the shared global `body` / `*` base layer.

If you prefer the compatibility alias:

```tsx
import "@wedaster/ui/globals.css"
```
